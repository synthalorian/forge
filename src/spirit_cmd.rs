...
static PLANS: &[ReadingPlan] = &[
    ...
    ReadingPlan {
        ...
        readings: &[
            ...
            (40, "Romans", 9, 12),
            (41, "Romans", 13, 16),
            (42, "1 Corinthians", 1, 4),
            (43, "1 Corinthians", 5, 8),
            (44, "1 Corinthians", 9, 12),
            (45, "1 Corinthians", 13, 16),
            (46, "2 Corinthians", 1, 4),
            (47, "2 Corinthians", 5, 9),
            (48, "2 Corinthians", 10, 13),
            (49, "Galatians", 1, 3),
            (50, "Galatians", 4, 6),
            (51, "Ephesians", 1, 3),
            (52, "Ephesians", 4, 6),
            (53, "Philippians", 1, 4),
            (54, "Colossians", 1, 4),
            (55, "1 Thessalonians", 1, 5),
            (56, "2 Thessalonians", 1, 3),
            (57, "1 Timothy", 1, 3),
            (58, "1 Timothy", 4, 6),
            (59, "2 Timothy", 1, 4),
            (60, "Titus,Philemon", 1, 1),
            (61, "Hebrews", 1, 4),
            (62, "Hebrews", 5, 8),
            (63, "Hebrews", 9, 12),
            (64, "Hebrews", 13, 13),
...
fn word_plan(
    ...
    if list || (name.is_none() && !today && activate.is_none() && !progress) {
        ...
        let active_plan: Option<String> = conn
            .query_row(
                "SELECT name FROM reading_plans WHERE active = 1",
                [],
                |row| row.get(0),
            )
            .ok();
        ...
        println!(
            "{}",
            crate::theme::style_bold_header("Reading Plans", theme)
        );
        ...
        for plan in PLANS {
            ...
            println!(
                "  {} {} — {} days",
                if is_active {
                    crate::theme::style_success(marker, theme)
                } else {
                    crate::theme::style_muted(marker, theme)
                },
            ...
            if is_active {
                let day: u32 = conn
                    .query_row(
                        "SELECT current_day FROM reading_plans WHERE active = 1",
                        [],
                        |row| row.get(0),
                    )
                    .unwrap_or(1);
    ...
    if progress {
        let conn = plan_connect_db(cfg)?;
        let (pname, cday): (String, u32) = conn
            .query_row(
        ...
        println!(
            "  {} {} — Day {} of {}",
            crate::theme::style_label("Plan:", theme),
            crate::theme::style_value(&pname, theme),
            crate::theme::style_value(&cday.to_string(), theme),
            crate::theme::style_value(&plan.total_days.to_string(), theme)
        );
        println!(
            "  {} [{}] {}%",
            crate::theme::style_label("Progress:", theme),
            crate::theme::style_value(&bar, theme),
            crate::theme::style_value(&pct.to_string(), theme)
        );
        println!();
        return Ok(());
    }
    ...
    if let Some(reading) = plan.readings.iter().find(|r| r.0 == current_day) {
        ...
        println!(
            "  {} {} — Day {} of {}",
            crate::theme::style_bold_header(plan.name, theme),
            crate::theme::style_muted(
                &format!("Day {} of {}", current_day, plan.total_days),
                theme
            ),
        ...
        conn.execute(
            "UPDATE reading_plans SET current_day = ?1 WHERE name = ?2",
        ...
        if current_day < plan.total_days {
            println!(
                "  {} Day {} tomorrow",
                crate::theme::style_muted("→ Next:", theme),
