...
pub fn run(cfg: &Config, args: &RestoreArgs) -> Result<()> {
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                ...
                crate::theme::style_error(
                    &format!("Backup not found: {}", args.backup_id),
                    theme,
                )
            )
        })?;
    ...
    if args.dry_run {
        println!("{}", crate::theme::style_bold_header("Dry run — would restore the following backup:", theme));
        println!();
        println!("  {}  {}", crate::theme::style_label("ID:", theme), crate::theme::style_value(&entry.id.to_string(), theme));
        println!("  {}  {}", crate::theme::style_label("Repo:", theme), crate::theme::style_value(&entry.repo_name, theme));
        println!("  {}  {}", crate::theme::style_label("Archive:", theme), crate::theme::style_value(&entry.archive_path, theme));
        println!("  {}  {}", crate::theme::style_label("Size:", theme), crate::theme::style_value(&format_size(entry.size_bytes), theme));
        println!("  {}  {}", crate::theme::style_label("Branches:", theme), crate::theme::style_value(&entry.branch_count.to_string(), theme));
        println!("  {}  {}", crate::theme::style_label("Tags:", theme), crate::theme::style_value(&entry.tag_count.to_string(), theme));
        println!("  {}  {}", crate::theme::style_label("Commits:", theme), crate::theme::style_value(&entry.commit_count.to_string(), theme));
        println!("  {}  {}", crate::theme::style_label("Type:", theme), crate::theme::style_value(&format!("{:?}", entry.backup_type), theme));
        println!(
            "  {}  {}",
            crate::theme::style_label("Created:", theme),
            crate::theme::style_value(&entry.created_at.format("%Y-%m-%d %H:%M:%S UTC").to_string(), theme),
        );
        println!();
        println!("  {}  {}", crate::theme::style_label("Target dir:", theme), crate::theme::style_value(&output_dir, theme));
        if let Some(ref_name) = &args.ref_name {
            println!("  {} {}", crate::theme::style_label("Checkout ref:", theme), crate::theme::style_value(ref_name, theme));
        }
        return Ok(());
    }
    ...
    if is_dedup {
        println!("{}", crate::theme::style_info("Extracting from dedup archive...", theme));
        crate::archive::extract_dedup_archive(cfg, &entry.archive_path, &output_dir)
            .context("Failed to extract dedup archive")?;
        println!("  {}", crate::theme::style_success("Extraction complete", theme));
    } else {
        println!("{}", crate::theme::style_info("Verifying archive integrity...", theme));
        let valid = crate::archive::verify_archive(&entry.archive_path, &entry.sha256)
            .context("Failed to verify archive integrity")?;
        if !valid {
            bail!("{}", crate::theme::style_error("Archive corrupted: SHA-256 mismatch", theme));
        }
        println!("  {}", crate::theme::style_success("SHA-256 verified OK", theme));
    ...
    if let Some(ref_name) = &args.ref_name {
        ...
        let bare_path_str = if bare_repo_path.exists() {
            ...
            println!(
                "  {}",
                crate::theme::style_muted(&format!("Found bare repo at {}", found.path().display()), theme),
            );
            found.path().to_string_lossy().into_owned()
        };

        clone_and_checkout(&bare_path_str, &output_dir, &entry.repo_name, ref_name, theme)?;
    }
...
fn clone_and_checkout(
    ...
    println!("  {}", crate::theme::style_info("Cloning bare repo to working directory...", theme));
